#!/usr/bin/env python
#
# Minneapolis Solar Powered Weather Station version 1.0
# A Punk-a-do memorial project (c)
#
...
def sampleSystemStats():
	...
	try:
        ...
        con = mdb.connect('localhost', 'root', conf.MySQL_Password, 'Weatherpi');
        cur = con.cursor()
        print "before query"
...
def get_weather_pid():
	...
	for i in plist: # loop through all pids
		if str(psutil.Process(i).name()) == name: # We only care about python processes
			...
			if "NewWP.py" in cmdline[1]: # check second cmd name for our process
...
def sampleSunAirPlus():
	...
	batteryCharge = returnPercentLeftInBattery(batteryVoltage, 4.10)
...
def writeWeatherRecord():
	...
	global as3935LightningCount,  as3935LastInterrupt, as3935LastDistance, as3935LastStatus
...
print ""
print "Minneapolis Weather Station version 1.0"
print ""
print ""
print "Program Started at:"+ time.strftime("%Y-%m-%d %H:%M:%S")
print ""

pclogging.log(pclogging.INFO, __name__, "WeatherPi Startup Version 1.0")
...
#			print "exception - as3935 I2C did not work"
	...
	if ((secondCount % (5*60)) == 0):
		...
		if (batteryVoltage < 3.45):
			print "--->>>>Time to Shutdown Battery is Low<<<<---"
			shutdownPi("low voltage shutdown")

	# every 15 minutes, build new graphs and stuff system data to mysql
